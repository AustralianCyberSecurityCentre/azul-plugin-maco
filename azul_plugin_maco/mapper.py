...
class ClearDict:
    ...
    def keys(self) -> Set[str]:
        """Get all keys that are not marked for deletion."""
        keys = set()
        for k in self.d.keys():
            if k not in self.to_delete:
                keys.add(k)
        return keys
...
def map_maco_config(mapped: MappedData, cd1: ClearDict, is_child: bool):
    """Map extractor config features to Azul features."""
    # Family is required for the main binary, but not for any children
    ...
    # encryption (child encryption handled seperately)
    if not is_child:
...
def map_child_maco_features(d: dict) -> Tuple[MappedData, dict]:
    """Map maco specific features from a child to Azul features."""
    mapped = MappedData()

    with ClearDict(d) as cd1:
        map_maco_config(mapped, cd1, True)

        remaining_features = {}
        for k in cd1.keys():
            remaining_features[k] = cd1.get(k, None)
        ...
        for feat_name, feat in list(mapped.features.items()):
            ...
            if isinstance(feat, list):
                mapped.features[feat_name] = sorted(
                    {x for x in feat}, key=lambda f: f.value if isinstance(f, FV) else f
                )
