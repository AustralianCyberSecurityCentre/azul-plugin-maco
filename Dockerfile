...
# Using bookworm because some maco libraries are old and don't support trixie yet.
...
RUN pip install \
    ...
    azul-plugin-maco==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-plugin-maco==$(cd /tmp/src && python -m setuptools_scm);fi
